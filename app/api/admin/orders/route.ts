...
export async function GET() {
  ...
  const payload = (orders ?? []).map((order) => ({
    ...
    shippingDiscountAmount: Number(order.shipping_discount_amount ?? 0),
    total: Number(order.total ?? 0),
    status: order.status,
    scheduledDate: order.scheduled_date,
    appliedRewardTitle: order.applied_reward_title,
    pointsEarned: Number(order.points_earned ?? 0),
    bonusPointsEarned: Number(order.bonus_points_earned ?? 0),
